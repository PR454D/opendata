...
mod tests {
    ...
    async fn should_handle_empty_label_selector() {
        ...
        // given: create a mock reader with series having different label states:
        // - series 1: has foo="bar" (should NOT match foo="")
        // - series 2: has foo="" (should match foo="")
        // - series 3: has no foo label (should match foo="")
        let bucket = TimeBucket::hour(1000);
        let mut builder = MockQueryReaderBuilder::new(bucket);
        ...
        // Series without foo label at all
        builder.add_sample(
            vec![
                Label {
                    name: METRIC_NAME.to_string(),
    ...
    #[test]
    fn should_handle_empty_label_selector_in_memory() {
        // given: create indexes with series having different label states
        let forward = ForwardIndex::default();
        let inverted = InvertedIndex::default();

        // Series with foo="bar" (should NOT match foo="")
        ...
        // Series with foo="" (should match foo="")
        ...
        // Series without foo label at all (should match foo="")
        ...
        assert!(result.contains(&2)); // foo=""
        assert!(result.contains(&3)); // no foo label
        assert!(!result.contains(&1)); // foo="bar" should NOT match
    }

    #[test]
    fn should_handle_multiple_empty_label_selectors() {
        // given: create indexes with series having different label combinations
        let forward = ForwardIndex::default();
        let inverted = InvertedIndex::default();

        // Series with foo="bar", baz="value" (should NOT match)
        let series1_labels = vec![
            Label {
                name: METRIC_NAME.to_string(),
        ...
        // Series with foo="", baz="" (should match)
        let series2_labels = vec![
            Label {
                name: METRIC_NAME.to_string(),
                value: "test_metric".to_string(),
        ...
        // Series with neither foo nor baz labels (should match)
        let series3_labels = vec![
            Label {
                name: METRIC_NAME.to_string(),
                value: "test_metric".to_string(),
        ...
        // then: should find 2 series (both empty and both missing), but NOT the one with non-empty values
        assert_eq!(
            result.len(),
            2,
            "Should find 2 series matching both conditions"
        );
        assert!(result.contains(&2)); // foo="", baz=""
        assert!(result.contains(&3)); // no foo or baz labels
        assert!(!result.contains(&1)); // foo="bar", baz="value" should NOT match
    }
